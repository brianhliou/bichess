...
//   node_modules/.bin/tsx scripts/variant-lab/jungle-puzzle-miner.ts --random-games 400 --unique-solutions --limit 24 --emit-seed > mined.json
//
// Storage (#183): puzzle content lives in the committed seed asset
// (packages/game/seed/puzzles/jungle.json) + the server's `puzzles` table, not
// TS modules. --emit-seed prints this run's puzzles as seed-format entries
// (one per line); merge them into the seed's mined section (replace the
// entries whose ids this miner owns, keep tactics/material entries), then
// update the pins in packages/game/src/puzzles-seed.test.ts and re-run
// `npm run test:puzzles:corpus`.
...
if (options.emitSeed) {
  ...
  console.error(
    'merge these entries into packages/game/seed/puzzles/jungle.json (mined section), then update puzzles-seed.test.ts pins',
  );
...
// packages/game/seed/puzzles/jungle.json carries. Printed as a JSON array to
// merge into the seed's `puzzles` list (this miner owns the mined forced-win
// section; tactics/material entries come from their own miners).
...
Options:
  ...
  --emit-seed             Print seed-format entries (one JSON puzzle per line) to merge into
                          packages/game/seed/puzzles/jungle.json (#183).
