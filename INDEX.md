...
| `jieqi-analysis.ts` | Whole-game "Computer analysis" for Jieqi (Reveal Xiangqi): mirrors `banqi-analysis.ts` but on Pikafish — deal-threaded reconstruction (`createInitialJieqiState(deal)`+`applyJieqiMove`) + redacted FEN (`jieqiStateToPikafishFen`, `evaluateJieqiFen`; Pikafish already emits `info score`, no engine change). Red-seat POV. Exposes `jieqiChancePlies` (a reveal is a face-down piece MOVING, not a from===to flip). Fail-closed + vacuous guard. Served by `routes/jieqi-games.ts` |
...
| `routes/jieqi-games.ts` | Jieqi postgame/review API branch; keeps non-chess finished games out of generic chess replay APIs, plus the `/analysis` endpoint (GET cached / POST account-gated compute, via `jieqi-analysis.ts`; replays the raw event log to recover the hidden deal) |
...
| `game-table.ts` | Shared right-column game-table DOM contract for live rooms and Mistboard TV: player rows, replay controls, move region, actions, clocks, captures, and typed element refs. Routes own behavior and populate the shared slots. |
...
| `jungle-flip-board.ts` | Interactive Flip Jungle board factory for the analysis/review tree (`review/jungle-flip-review.ts`): tap-to-flip / select-and-move on `renderJungleFlipBoardSvg` via the shared `installBoardDrag` (inline flip-priority click logic mirroring `banqiClickResult`). Symmetric hidden-deal on 4×4 animals |
...
| `banqi-board.ts` | Interactive Flip Xiangqi board factory for the analysis/review tree (`review/banqi-review.ts`): tap-to-flip / select-and-move on `renderBanqiBoardSvg` via the shared `installBoardDrag`, reusing the pure `banqiClickResult`. Symmetric hidden-deal on 8×4 |
...
| Agent/dev loop | `agent-scan.mjs`, `dev.mjs`, `product-profile.mjs`, `worktree-role.mjs`, `worktree-local-files.mjs`, `ci-browser-smoke-plan.mjs`, `ci-checks.mjs`, `drift-check.mjs`, `gate-evidence.mjs`, `verify.mjs`, `worktree-new.mjs`, `worktree-prepare.mjs`, `mobile-loop.mjs`, `visual-check.mjs`; `config/product-profile.json` is the product-shaped local variant list shared by dev and QA seeding |
